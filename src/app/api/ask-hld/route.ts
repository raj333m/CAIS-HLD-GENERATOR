  ...
  try {
    ...
    if (matchesAny(qLower, ['what is cais and why does hsbc', 'what is cais', 'why does hsbc report'])) {
      if (isSectionWritten('1.1')) {
        ...
        answer = 'Section 1.1 is currently unwritten ([Content to be confirmed by BA — no verified source provided yet]). This information is not yet documented in the active HLD.';
      }
    ...
    // 3. "What does the CU (Central Utility) Team own in this process?"
      ...
      if (isSectionWritten('1.1') && isSectionWritten('1.2')) {
        ...
        answer = 'Section 1.1/1.2 is currently unwritten ([Content to be confirmed by BA — no verified source provided yet]). This information is not yet documented in the active HLD.';
      }
      suggestedNextQuestions = [
        'Which six CAIS variables are calculated by the CU Team?',
    ...
    // 4. "Why is HSBC Harvey Nichols not currently reporting to the bureaus?"
    else if (matchesAny(qLower, ['harvey nichols', 'harvey', 'nichols'])) {
      if (isSectionWritten('1.3')) {
        ...
        answer = 'Section 1.3 is currently unwritten ([Content to be confirmed by BA — no verified source provided yet]). This rule is not yet documented in the active HLD.';
      }
      suggestedNextQuestions = [
    ...
    // 5. "What is the Standing Exclusion Rule that applies to all brands?"
    ...
    // 6. "Which six CAIS variables are calculated by the CU Team?"
      ...
      if (isSectionWritten('1.2')) {
        ...
        answer = 'Section 1.2 is currently unwritten ([Content to be confirmed by BA — no verified source provided yet]). This information is not yet documented in the active HLD.';
      }
      suggestedNextQuestions = [
        'Why can\'t these variables be sourced directly from the operational systems?',
        'When in the monthly cycle does the CU Team\'s calculated data become available?',
    ...
    // 8. "What's different between HSBC Retail and First Direct Retail eligibility criteria?"
      ...
      if (isSectionWritten('1.2') && isSectionWritten('1.3')) {
        ...
        answer = 'Section 1.3 is currently unwritten ([Content to be confirmed by BA — no verified source provided yet]). This information is not yet documented in the active HLD.';
      }
      suggestedNextQuestions = [
    ...
    // Other specific topics
      ...
      if (isSectionWritten('1.1')) {
        answer = 'CAIS reporting files are delivered via Connect:Direct to Experian UK (cut-off: 14th of each month, 17:00 GMT), Equifax UK (cut-off: 15th of each month, 12:00 GMT), and TransUnion UK (cut-off: 18th of each month, 18:00 GMT).';
        citations.push({
          sectionNumber: '1.1',
          title: '1.1 Business Overview and Requirements Summary',
          heading: 'Credit Reference Agencies in the UK',
    ...
    // Fallback keyword search across database
      ...
      if (keywords.length > 0) {
        ...
        for (const sec of sections) {
          if (!isSectionWritten(sec.sectionNumber)) continue;
